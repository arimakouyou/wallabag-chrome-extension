...
function setupEventListeners() {
    ...
    if (elements.toggleSecretBtn) {
        elements.toggleSecretBtn.addEventListener('click', () => togglePasswordVisibility('client-secret'));
    }
    if (elements.togglePasswordBtn) {
        elements.togglePasswordBtn.addEventListener('click', () => togglePasswordVisibility('password'));
    }
...
async function handleImportConfig(event) {
    ...
    try {
        ...
        // 設定の適用
        if (importData.config.serverUrl) elements.serverUrl.value = importData.config.serverUrl;
        if (importData.config.clientId) elements.clientId.value = importData.config.clientId;
        if (importData.config.username) elements.username.value = importData.config.username;
        ...
        elements.showNotifications.checked = importData.behavior.showNotifications !== false;
        
        showMessage('success', '設定がインポートされました。保存ボタンをクリックして適用してください。');
...
async function handleClearData() {
    if (!confirm('すべての設定とデータを削除しますか？この操作は元に戻せません。')) {
        return;
    }
...
function validateConfig(config) {
    ...
    if (!config.clientSecret?.trim()) errors.push('クライアントシークレットは必須です');
